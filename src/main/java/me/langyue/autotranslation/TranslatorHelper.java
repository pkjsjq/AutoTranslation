...
//        }
     ...
     * @return 翻译后的文本
     */
    private static int translateSyncCallCount = 0;

    public static String translateSync(String key, String en) {
        if (CACHE.containsKey(key)) {
            return CACHE.get(key);
        }
        translateSyncCallCount++;
        ITranslator translator = TranslatorManager.getTranslator();
        if (translateSyncCallCount <= 5) {
            AutoTranslation.LOGGER.info("[DEBUG] translateSync #{}: translator={}, ready={}, key={}, en={}",
                translateSyncCallCount, translator.getClass().getSimpleName(), translator.ready(), key, en);
        }
        String translation = translator.translate(en, AutoTranslation.getLanguage(), "en");
