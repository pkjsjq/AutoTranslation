...
The core feature. When Minecraft loads resources, AutoTranslation detects language entries that are missing a translation for your current game language, translates them using the configured engine, and injects the translations at runtime — all without modifying any mod files.

- Works with **any mod** that uses standard Minecraft language files (`.json`)
- Translated files are saved in the `AutoTranslation/` folder under your game directory, where you can review, polish, and even repackage them into a resource pack
...
3. Download the latest AutoTranslation `.jar` from [Modrinth](https://modrinth.com/mod/autotranslation) (recommended) or [CurseForge](https://www.curseforge.com/minecraft/mc-mods/autotranslation)
...
Open **Mods** → **AutoTranslation** → **Config** (requires Cloth Config API) to access all settings in-game. Alternatively, edit the config file at `config/autotranslation.json`.
...
AutoTranslation has an extensible translator architecture. To add a custom translation engine:
...
1. When Minecraft loads resources, AutoTranslation intercepts the language loading process
...
4. Translated results are cached in the `AutoTranslation/` folder under your game directory
5. On subsequent launches, cached translations are loaded instantly — no re-translation needed

The `AutoTranslation/` folder contains:
...
AutoTranslation 正是为解决这些痛点而生——它在游戏加载资源时自动检测并翻译缺失的语言条目，**不修改任何模组文件**，翻译结果缓存在本地，可随时润色、导出为资源包。
...
- 翻译结果保存在游戏目录的 `AutoTranslation/` 文件夹中，可手动润色后打包为资源包——对整合包和汉化组尤其有用
...
3. 从 [Modrinth](https://modrinth.com/mod/autotranslation)（推荐）或 [CurseForge](https://www.curseforge.com/minecraft/mc-mods/autotranslation) 下载最新版 AutoTranslation
...
在游戏中打开 **Mods** → **AutoTranslation** → **Config**（需安装 Cloth Config API）即可可视化配置。也可直接编辑 `config/autotranslation.json` 配置文件。
...
AutoTranslation 拥有可扩展的翻译器架构。添加自定义翻译引擎仅需两步：
...
1. Minecraft 加载资源时，AutoTranslation 拦截语言加载流程
2. 对每个缺少当前语言翻译的条目，使用可配置的正则表达式判断是否需要翻译
3. 匹配的条目发送至指定的翻译引擎进行翻译
4. 翻译结果缓存在游戏目录的 `AutoTranslation/` 文件夹中
5. 下次启动时直接从缓存加载，无需重复翻译

`AutoTranslation/` 文件夹内容：
- **原始文件** — 未翻译的源文本
- **翻译文件** — 机翻输出
